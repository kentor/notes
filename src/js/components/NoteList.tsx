...
import Note from 'App/components/Note.tsx';
import NoteForm from 'App/components/NoteForm.tsx';
import React, { useState } from 'react';
import { logout } from 'App/api.ts';
...
function NoteListInner({ notes }: { notes: Array<NoteT> }) {
  ...
  const queryRegExp = trimmedQuery &&
    new RegExp(escapeRegExp(trimmedQuery), 'i');
      ...
      >
        ...
        {notes.map((note) => (
          <Note
            key={note.id}
            note={note}
            visible={(showHiddenOnly ? note.hidden : true) &&
